...
sub argv ()
{
   ...
   # If this number is not 2, die:
   if (2 != $NA)
   {
      warn "Error: Must have 2 arguments, which must be directory paths,\n"
         ...
         . "any files. Run this program with -h or --help option to get more help.\n";
      exit;
   }
   ...
   if ( ! -e e $srcedire ) {die "Error: source directory doesn't exist.         \n$!\n"}
   if ( ! -d e $srcedire ) {die "Error: source directory isn't a directory.     \n$!\n"}
   if ( ! -e e $destdire ) {die "Error: destination directory doesn't exist.    \n$!\n"}
   if ( ! -d e $destdire ) {die "Error: destination directory isn't a directory.\n$!\n"}
   my $dhs = undef;
   my $dhd = undef;
   if ( ! opendir($dhs, e $srcedire) )
                           {die "Error: couldn't open source directory.         \n$!\n"}
   if ( ! opendir($dhd, e $destdire) )
                           {die "Error: couldn't open destination directory.    \n$!\n"}
   my @dest = d readdir $dhd;
   for (@dest)
   {
      if ('.' ne $_ && '..' ne $_)
      {
                            die "Error: destination directory isn't empty.      \n$!\n"
      }
   }
   closedir $dhs;
   closedir $dhd;
   ...
   chdir e $srcedire     or die "Error: couldn't chdir to source directory.     \n$!\n";
   $srcedire = d getcwd;
   $srceleng = length $srcedire;
   if ($db2) {say "source directory = $srcedire";}
   chdir e $strtdire     or die "Error: couldn't chdir to starting directory.   \n$!\n";
   chdir e $destdire     or die "Error: couldn't chdir to destination directory.\n$!\n";
   $destdire = d getcwd;
   if ($db2) {say "destination directory = $destdire";}

   # IMPORTANT: chdir one more time, back to srcedir, or we'll start at the wrong place!!!
   chdir e $srcedire     or die "Error: couldn't chdir to source directory.     \n$!\n";
