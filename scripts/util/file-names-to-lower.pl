...
########################################################################################################################
# file-names-to-lower.pl
# Converts names of all files in current directory to all-lower-case.
#
# Edit history:
...
sub process_argv ()
{
   ...
   given ($NA)                         # Given the number of arguments,
   {
      ...
      when (1) {$Regexp = $CLArgs[0];} # if $NA == 1, set $Regexp;
...
sub process_current_file ($)
{
   ...
   my $newpref = lc $oldpref;          # convert prefix to lower-case
   ...
   {
      my $success = rename_file($oldname, $newname);
      if ($success)
      {
         say("$oldname -> $newname");
         ++$renacount;
...
sub stats ()
{
   print("\nStatistics for program \"file-names-to-lower.pl\":\n");
...
sub help ()
{
   ...
   Welcome to "file-names-to-lower.pl". This program converts the names of all
   files in the current directory (and all subdirectories if a -r or --recurse
   option is used) to all-lower-case.

   Command line:
   file-names-to-lower.pl [-h|--help] [-r|--recurse] [Arg1]
