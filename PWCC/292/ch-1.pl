...
element in the array is at least twice as big as every element in
the given array. If it is, return the index of the largest
...
--------------------------------------------------------------------------------------------------------------
PROBLEM NOTES:
First I'll need a sub to make sure that a given array ref actually IS a ref to an array of integers with a
unique largest element.

Then, I'll need a sub to determine if the largest element is greater-than-or-equal-to twice the
second-largest.
...
$"=', ';
for my $aref (@arrays) {
   my ($lv,$li,$tl) = twice_or_more($aref);
   say '';
   say "Array = (@$aref)";
   say "Largest element is $lv at index $li.";
   say "Largest element ", $tl ? "IS" : "is NOT", " at least twice as large as next-largest element.";
}
