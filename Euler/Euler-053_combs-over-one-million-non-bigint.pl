...
###############################################################################
# /rhe/scripts/Euler/Euler-0053_combs-over-one-million-non-bigint.perl
...
# main
{
   ...
   while ($n <= 100)
   {
      ...
      # Therefore, all the numbers on this row of Pascal's triangle will be
      # greater than 1,000,000 EXCEPT FOR $k numbers on the left, and
      # $k numbers on the right. (Remember, now, the rows are zero-indexed,
      # so the number of numbers to the left of index $k is $k, NOT $k-1.)
      # This is because every row of Pascal's triange is symmetric and 
      # palindromic. So if $k <= $n/2, the number of numbers on the current
      # row which are over 1,000,000 will be ($n+1)-2*$k.
      else
      {
