#!/usr/bin/perl
# /rhe/scripts/math/Euler-0011_4-in-row.perl
# Finds greatest product of 4 adjacent number in 20x20 grid below
# (where "adjacent" can mean horiz, vert, or diag).
# Edit history:
#    Wed Feb 17, 2016 - Wrote it.
...
my $GP = 0; # Greatest Product
my $TV = 0; # Temporary Value

foreach my $row (0..19)
{
   foreach my $col (0..19)
   {
      $TV = Diau($row, $col); $GP = $TV if $TV > $GP;
      $TV = Righ($row, $col); $GP = $TV if $TV > $GP;
      $TV = Diad($row, $col); $GP = $TV if $TV > $GP;
      $TV = Down($row, $col); $GP = $TV if $TV > $GP;
   }
}
