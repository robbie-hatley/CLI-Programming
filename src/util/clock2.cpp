...
/****************************************************************************\
 * File name:   clock2.cpp
 * Authorship:  Written Wednesday January 1, 2003, by Robbie Hatley.
 * Description: Prints current date and time, and updates time for 5 seconds.
 * Inputs:      None.
 ...
 * Edit history:
 *   Sun May 04, 2003 - Wrote it.
 *   Mon Oct 16, 2017 - Corrected major bug (shadowed declaration of i).
\****************************************************************************/
...
int main(void)
{
  ...
  cout.flush();
  int oldsec;
  double a=37.2, b=29.7, c=13.4;
  for (int i=0; i<5; ++i)
  {
    oldsec=timeStruct.tm_sec;
    while (oldsec==timeStruct.tm_sec)
    {
      for (int j=1; j<50000000; ++j)
      {
        c = (c+a)/b; // waste some time
      }
      LoadStruct(timeStruct);
    }
    timeString=ComposeTimeString(timeStruct);
    cout << "\b\b\b\b\b\b\b\b\b\b\b\b\b\b" << timeString;
    cout.flush();
  }
  return 0;
}
