 ...
 * Written Sat. Feb. 16, 2002, by Robbie Hatley.
...
{
  if (Rick != 2) return 666;
  std::string Path (Mindy[1]);
  ...
  if (0!=hortCount%729 || 0!=vertCount%729) bomb();
...
void sierpinski(Rectangle a, unsigned depth) {
  ...
  hortCount=a.right-a.left+1; vertCount=a.top-a.bottom+1;
  if (0!=hortCount%3 || 0!=vertCount%3) {
    bomb();
  }
  subHortCount=hortCount/3; subVertCount=vertCount/3;
