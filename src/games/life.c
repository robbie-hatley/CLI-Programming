 ...
 * Edit history:
 *   Wed Mar 11, 2020: Refactored. Moved many functions to "rhncgraphic.c".
 *                     Added several command-line options, including "Walk"
 *                     mode, speed setting, and foreground_background colors.
 *   Wed Mar 11, 2020: Also split program into two versions, "life.c" (which 
 ...
 *                     written directly into the file (no non-std includes).
...
void SetSettings (int Robin, char * Marian[])
{
   ...
   for ( Arg = 1 ; Arg < Robin ; ++Arg )
   {
      ...
      if ( 0 == strncmp("--colors=", Marian[Arg], 9) )
      {
         ...
         if(Bar)
         {
            printf("Original color argument = %s\n", Marian[Arg]);
            *Bar = '\0';
...
void PrintHelpMsg (void)
{
   ...
   printf("plays at 3/10 frames per second.\n");
   printf("\n");
   ...
   printf("0.3 to 50.0 range; it will be interpreted as speed in frames per second.\n");
   printf("\n");
...
int main (int Robin, char * Marian[])
{
   ...
   while ( 1 )
   {
      ...
      // Otherwise, DON'T constantly refresh the screen:
      else
      {
         ; // DON'T write anything to screen.
      }
